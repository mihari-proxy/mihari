...
> 历史设计说明：日志级别同步与固定 stdout/stderr 映射已由 [2026-09-16 统一日志设计](2026-09-16-unified-logging-level-design.md) 取代；日志原文策略继续以 2026-09-14 完整错误日志设计为准。下文保留当时的设计记录。

日期：2026-09-02

后续策略替代：本文中的日志内容脱敏规则由[2026-09-14 完整错误日志设计](2026-09-14-full-error-logging-design.md)替代。新策略保留文件、快照和导出原文，导出页面红色说明；既有权限、轮转、原子发布与公开输出边界继续适用。本文其余内容保留历史设计背景。
