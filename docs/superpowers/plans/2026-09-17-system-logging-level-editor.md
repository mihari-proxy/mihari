...
- [Pullfrog 首轮 review](https://github.com/mihari-proxy/mihari/pull/270#pullrequestreview-5230943194) 认可整体交互，仅建议解释 SILENT 防护。补充注释与 `TestLoggingLevelEdit_PassiveSilentCannotBeSubmittedAfterExternalChange`：外部从 SILENT 改为 INFO 时，未切换的被动候选仍不能主动提交。临时删除防护可使测试失败，恢复后通过；TUI 全部 race 测试与全仓 lint 再次通过。该反馈不需要改变已确认的业务行为。
