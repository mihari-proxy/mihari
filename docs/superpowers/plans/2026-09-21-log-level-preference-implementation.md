...
- `fd86bf2` 的 CI 和 Pullfrog 全量复审已通过；等待期间 `dev` 合入 #293，随后再次 rebase 到 `bf3e998`。合并偏好服务、DTO、installer decoder 和首次加载入口的冲突，保留 Proxies 设置与 Logs Level 的独立更新；补充三类设置交替保存及联合迁移文件的回归覆盖，重新验证并等待新 head 的 CI 与评审。
