...
Windows updates drain accepted daemon writes before stopping related instances. A manually started daemon remains stopped: start it again using your original `mihari daemon` invocation, then enter `mihari`. Older or non-elevated manual daemons without preparation support must be closed normally before updating. Normal startup cleans obsolete binaries in the background; failures remain available in F2 without blocking startup. The first update initiated by an older version still uses that version's exit behavior; close its old sessions and enter `mihari` again if needed.
