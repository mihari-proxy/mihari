...
- **Automatic version checks**: entering System checks the core's selected channel; entering Web GUI checks every supported panel, including uninstalled ones. Core and Mihari checks use an animated `Checking` badge, then show the latest version/build, `Up to date`, or `Check failed`; these System checks do not block other System actions. Successful core and panel checks are cached for five minutes within the TUI session; the Mihari check runs again each time System is opened. Failed checks retry on page re-entry. Successful core installs/channel switches and panel install/update/rollback/reinstall/uninstall actions immediately refresh the affected version check. Checks only fetch metadata; installation still requires confirmation.
