...
- **Outbound interface override**: choose an adapter in **System → Network → Outbound Interface Override**, or use `mihari egress list|status|set <name>|auto`. The System row shows the current selection in bright yellow. Use ↑/↓ to browse the scrolling list, PgUp/PgDn for details, and Enter to use the highlighted row. Unavailable saved adapters remain visible without automatic fallback. A running core reloads and closes tracked active connections; a stopped core only saves the choice. No-Override restores the original subscription configuration. This uses native mihomo bindings and does not guarantee two global TUNs can coexist. Before downgrading to an older Mihari, switch back to No-Override (`mihari egress auto`) to remove the new settings field.
...
- **In-TUI Mihari updates**: the System page checks GitHub Releases when the rail selects System, before Enter, shows `current -> latest available` or `current · Up to date`, and—when Mihari was started with administrator/root privileges—replaces the binary, synchronizes and restarts an installed OS-service copy, verifies its daemon version, and automatically enters the updated TUI. The update confirmation keeps safe nonstandard installed build labels as `Unknown[label]`; compatibility remains unknown. Long confirmation content scrolls with ↑/↓ or PgUp/PgDn, with Cancel selected by default.
- **Core channel**: the System page can switch the mihomo core between `stable` and `alpha`.
- **Automatic version checks**: selecting System on the rail, before Enter, checks both Mihari and the core channel every time. Entering the page does not start another one of those checks and does not cancel a check already in progress. Entering Web GUI checks every supported panel, including uninstalled ones. Core and Mihari checks use an animated `Checking` badge, then show `current -> latest available`, `Up to date`, or `Check failed`; these System checks do not block other System actions. A Web GUI panel check shows that same badge on Latest while it is running. Successful panel checks are cached for five minutes within the TUI session. A failed Mihari or core check retries the next time the rail selects System. Successful core installs/channel switches and panel install/update/rollback/reinstall/uninstall actions immediately refresh the affected version check. Checks only fetch metadata; installation still requires confirmation.
...
Selected proxy cards use a blue **●** marker in the same color as **INFO** log entries. Long candidate names wrap within each card instead of being truncated; all cards in a row match the tallest card in that row. Each Proxies group, including GLOBAL, has **→ Jump to Selected** beside its current selection (shortened to **→ Selected** in narrow windows). On the group header, press → to focus the button and Enter to expand the group and scroll to its selected card; ← returns to the header. This only moves keyboard focus. Later refreshes do not move that focus when the selection changes. The button remains usable with retained **Last selected** data, and is disabled when the selection is empty or absent from the candidate list.

In TUI **Subs**, Enter opens editable details; `a` adds a profile. Use Tab/Shift+Tab or ↑/↓ to move between fields and actions, and ←/→/Space to cycle **Auto refresh** or **Mode**. Enter on a settings field advances to the next field; Enter on **Save** submits settings. The **Enabled** action enables or disables the subscription immediately; **InUse → Use this subscription** selects an enabled subscription with a valid cache. An already selected subscription shows **In use**. Disabling it also clears InUse; enabling it does not select it automatically. Actions keep details open and preserve unsaved settings. PgUp/PgDn scroll the details; long URLs scroll horizontally. All built-in TUI text is English. The list uses **InUse**, **Enabled**, **Status**, and **Mode**; `p` cycles fetch mode.
...
Adding and editing share a compact centered form. Details group the live status above the settings; empty errors stay hidden and timestamps use local time to the minute. In short windows the body scrolls while **Save** stays visible. Auto refresh, Mode, Enabled, and InUse highlight only the focused option, keeping labels and trailing space unhighlighted; text fields use a lightened input background and white cursor. Contextual shortcuts appear once at the bottom of the terminal. **Interval** shows the supported units (`ns/us/ms/s/m/h`) in muted text beside its input. An empty input shows the current global interval as a placeholder; leaving it blank keeps inheritance.
