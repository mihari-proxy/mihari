...
func TestSearch_CtrlFDoesNotStealDetailFocus(t *testing.T) {
	m := New(nil, nil)
	m.detail = NewDetail(protocol.Connection{ID: "one"}, false)
	_, cmd := m.Update(tea.KeyPressMsg{Code: 'f', Mod: tea.ModCtrl})
...
func TestSearch_CtrlFDoesNotStealColumnsFocus(t *testing.T) {
	m := New(nil, nil)
	m.openColumns()
	_, focused := m.FocusSearch()
	_, cmd := m.Update(tea.KeyPressMsg{Code: 'f', Mod: tea.ModCtrl})
	if focused || !m.columnsOpen || m.searching || cmd != nil {
		t.Fatal("Ctrl+F escaped columns")
	}
}
