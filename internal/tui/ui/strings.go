...
const (
	...
	ResetUserDataLabel      = "Reset user data"
	ResetUserDataDetail     = "Clear subscriptions, settings, and setup state, then start Mihari from scratch."
	ResetUserDataTitle      = "Reset user data"
	ResetUserDataImpact     = "This cannot be undone. Mihari will disable its system proxy and TUN, then delete settings, subscriptions, generated config, TUI preferences, panel activation, logs, and staging files. The Mihari binary, OS service, control token, core, GeoIP, and installed panel builds are kept. Setup will run again."
	ResetUserDataRollback   = "Disk state is not restored if this action fails partway through."
	ResetUserDataFailed     = "Reset user data failed"
	ResetUserDataProgress   = "Resetting"
	...
	DowngradeMihariTitle            = "Install older Mihari"
	DowngradeMihariImpact           = "This replaces the running Mihari with an older version. Settings, subscriptions, and generated files written by the current version may be unsupported, fail to load, or look like they disappeared. Downgrade is not a supported config migration and does not roll disk state back."
	DowngradeMihariRollback         = "The current TUI stays open if replacement does not complete. Disk state is not reverted."
