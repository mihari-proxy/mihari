...
func DownloadMigrationCore(ctx context.Context, client *http.Client, goos, arch, tag string) (binary, receipt []byte, resultErr error) {
	a, err := supportedCore(ctx, goos, arch, tag, "stable")
	if err != nil {
		return nil, nil, err
	}
	...
	if err != nil {
		return nil, nil, coreHTTPError(protocol.APIError{Code: protocol.CodeDataFailure, Message: "download supported migration core"}, "core GET migration asset", a.URL, "transport", nil, err)
	}
	defer closeCoreResponse(ctx, response, "core GET migration asset", a.URL, &resultErr, nil)
	if response.StatusCode != http.StatusOK {
		return nil, nil, coreHTTPError(protocol.APIError{Code: protocol.CodeDataFailure, Message: "download supported migration core"}, "core GET migration asset", a.URL, "response", response, nil)
	}
...
func rebuildMigrationCore(ctx context.Context, a supportedAsset, archive []byte) ([]byte, []byte, error) {
	...
	if err != nil {
		return nil, nil, dataFailureCause("invalid trusted core archive", err)
	}
	defer reader.Close()
	binary, err := io.ReadAll(io.LimitReader(reader, maxCoreBinarySize+1))
	if err != nil {
		return nil, nil, dataFailureCause("invalid trusted core binary", err)
	}
